...
async function refreshPurchasedBooks(visibleBooks) {
    ...
    visibleBooks.forEach(book => {
        ...
        if (book.hasPdf ?? Boolean(book.pdfFile)) {
            ...
            message.innerHTML = `<a class="btn buy-book" href="reader.html?id=${encodeURIComponent(book._id)}" target="_blank">اقرأ الكتاب</a> · <a  href="${accessUrl}?download=1" target="_blank">تحميل PDF</a>`;
        }
    });
}
