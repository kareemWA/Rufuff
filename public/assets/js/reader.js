...
async function loadReader() {
    ...
    try {
        const bookResponse = await fetchWithTimeout(`/api/books/${encodeURIComponent(bookId)}`, { cache: "no-store" });
        ...
        const readResponse = await fetchWithTimeout(`/api/books/${encodeURIComponent(bookId)}/read`, { method: "POST" });
        ...
        showError(error.name === "AbortError" ? "استغرق تحميل الكتاب وقتًا طويلًا. حاول مرة أخرى." : error.message || "تعذر فتح الكتاب.");
    }
}
