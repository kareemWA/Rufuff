...
لا ترفع ملفات PDF أو صور الأغلفة إلى هذا المشروع أو إلى MongoDB. تستخدم لوحة الإدارة Supabase Storage، ثم تحفظ قاعدة البيانات رابط HTTPS فقط. أبقِ `SUPABASE_SERVICE_ROLE_KEY` في Environment Variables على الخادم فقط، ولا تضعه في الواجهة. يجب أن يكون bucket الخاص بالكتب مضبوطًا بسياسة وصول مناسبة، ويفضل استخدام Signed URLs لملفات PDF؛ الخادم يتحقق من شراء المستخدم قبل إصدار الوصول، بينما نقل الملف يتم من Supabase وليس من Vercel.
